...
class HttpUtils {
  ...
  static Future<Map<String, String>> headers() async {
    ...
    if (_customHttpHeaders.isNotEmpty) {
      ...
      _customHttpHeaders
          .clear(); // Clear custom headers after use if they are one-time
    } else {
    ...
    debugPrint(
        "HttpUtils.headers(): Retrieved JWT Token from local storage: $jwtToken"); // Debugging line

    if (jwtToken != null && jwtToken.isNotEmpty) {
      // Also check for emptiness
      headerParameters['Authorization'] = 'Bearer $jwtToken';
      debugPrint(
          "HttpUtils.headers(): Added Authorization Header: Bearer $jwtToken"); // Debugging line
    } else {
      headerParameters.remove('Authorization');
      debugPrint(
          "HttpUtils.headers(): No JWT Token found or it was empty. Authorization header NOT added."); // Debugging line
    }
  ...
  static Future<http.Response> postRequest<T>(String endpoint, T body,
      {Map<String, String>? headers}) async {
    debugPrint("BEGIN: POST Request Method start : API$endpoint");
    ...
    try {
      ...
      response = await client
          .post(url,
              headers: requestHeaders, body: messageBody, encoding: _encoding)
          .timeout(_timeout);
    ...
    debugPrint("END: POST Request Method end : API$endpoint");
    return response;
  }

  static Future<http.Response> getRequest(String endpoint,
      {String? pathParams, Map<String, String>? queryParams}) async {
    debugPrint("BEGIN: GET Request Method start : API$endpoint");
    ...
    debugPrint("END: GET Request Method end : API$endpoint");
    return response;
  }
  ...
  //         .get(Uri.parse('API$endpoint'), headers: headers)
  ...
  static Future<http.Response> putRequest<T>(String endpoint, T body) async {
    debugPrint("BEGIN: PUT Request Method start : API$endpoint");
    ...
    debugPrint("END: PUT Request Method end : API$endpoint");
    return response;
  }

  static Future<http.Response> patchRequest<T>(String endpoint, T body) async {
    debugPrint("BEGIN: PATCH Request Method start : API$endpoint");
    ...
    debugPrint("END: PATCH Request Method end : API$endpoint");
    return response;
  }

  static Future<http.Response> deleteRequest(String endpoint,
      {String? pathParams, Map<String, String>? queryParams}) async {
    debugPrint("BEGIN: DELETE Request Method start : API$endpoint");
    ...
    debugPrint("END: DELETE Request Method end : API$endpoint");
    return response;
  }
